...
if(isset($_POST['nome'])){
    extract($_POST);
    if($consulta = $conexao->query("update tb_fontes set fon_nome = '$nome', fon_logo = '$logo' , fon_confiabilidade = '$confiabilidade', fon_website = '$website', fon_pol_codigo = '$polarizacao' where fon_codigo = $codigo;")){
    header("Location: listafontes.php");    
	} else {
		echo "Erro na edição";
	}
}
$consulta2 = $conexao->query("select * from tb_fontes where fon_codigo = $codigo");
 ...
 <!--≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡ HEAD ≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡≡-->
  ...
  <body id="body-search-result">
    ...
    <div class="container mt-3" id="area-pesquisa"
      ...
      <form class="form" action="?codigo=<?php echo $resultado2['fon_codigo'] ?>" method="POST" id="form-search">
        <div class="form-group">

          <!--======----- Área de PESQUISA -----======-->
          <label for="search">Editar fonte:</label>
          <input 
          type="text" 
          class="form-control" 
          value="<?php echo $resultado2['fon_nome'] ?>"
          name="nome" 
          required 
          id="pesquisa" 
          ...
          Logo: <input type="text" name="logo" required value="<?php echo $resultado2['fon_logo'] ?>" id="pesquisa" class="form-control">
          Website: <input type="text" name="website" required value="<?php echo $resultado2['fon_website'] ?>" id="pesquisa" class="form-control">
         
          <label>Confiabilidade:</label>
          <input type="number" name="confiabilidade" required value="<?php echo $resultado2['fon_confiabilidade'] ?>" id="pesquisa" class="form-control">
      
          Polarização:
          <select name="polarizacao">
		        <?php while($resultado3 = $consulta3->fetch_assoc()){ ?>
			        <option value="<?php echo $resultado3['pol_codigo']?>"<?php if($resultado2['fon_pol_codigo'] == $resultado3['pol_codigo']) { echo "selected";} ?>><?php echo $resultado3['pol_polarizacao']?></option>
		        <?php } ?>
		  </select>
