<?php 
include("../config.php");
if(isset($_POST['polarizacao'])){
    extract($_POST);
    if($consulta = $conexao->query("insert into tb_polarizacoes (pol_polarizacao) values ('$polarizacao');")) {
	} else {
...
if(isset($_GET['excluir'])){
    $codigo = $_GET['excluir'];
    if($consulta2 = $conexao->query("delete from tb_polarizacoes where pol_codigo = $codigo")) {
        header("Location: lcpolarizacao.php");
    } else {
        echo "Erro na exclusão";
        }
    }
$consulta = $conexao->query("select * from tb_polarizacoes");
?>
    ...
    <div class="container mt-3" id="area-pesquisa"
      ...
      <form class="form" action="?" method="POST" id="form-search">
        <div class="form-group">
          ...
          <!--======----- Área de PESQUISA -----======-->
          <label for="search">Cadastrar nova polarização:</label>
          <input 
          type="text" 
          name="polarizacao"
          class="form-control" 
    ...
    <div class="container">
      <table class="table table-sm">
        ...
        <tr class="instrucoes">
          ...
          <th class="col-xs-8 col-sm-8 col-md-6" scope="col">Polarizações</th>
        ...
        <?php while($resultado = $consulta->fetch_assoc()){ ?>
            <tr class="result">
                <td><?php echo $resultado['pol_codigo']?></td>
                <td><?php echo $resultado['pol_polarizacao']?></td>
                <td class="editar" >
                    <a href="editarpolarizacao.php?codigo=<?php echo $resultado['pol_codigo'];?>">Editar</a>  
                    <a href="?excluir=<?php echo $resultado['pol_codigo'];?>">Excluir</a>
                </td>
            </tr>            
        <?php } ?>
         <!--======-- fim ex 1 --=======-->
