...
if(isset($_POST['rede'])){
    extract($_POST);
    if($consulta = $conexao->query("insert into tb_redes_sociais (rcs_rede_social) values ('$rede');")) {
	} else {
...
if(isset($_GET['excluir'])){
    $codigo = $_GET['excluir'];
    if($consulta2 = $conexao->query("delete from tb_redes_sociais where rcs_codigo = $codigo")) {
        header("Location: lcredes.php");
    } else {
        echo "Erro na exclusão";
        }
    }
$consulta = $conexao->query("select * from tb_redes_sociais");
?>
    ...
    <div class="container mt-3" id="area-pesquisa"
      ...
      <form class="form" action="?" method="POST" id="form-search">
        <div class="form-group">
          ...
          <!--======----- Área de PESQUISA -----======-->
          <label for="search">Cadastrar nova rede:</label>
          <input 
          type="text" 
          name="rede"
          class="form-control" 
    ...
    <div class="container">
      <table class="table table-sm">
        ...
        <?php while($resultado = $consulta->fetch_assoc()){ ?>
            <tr class="result">
                <td><?php echo $resultado['rcs_codigo']?></td>
                <td><?php echo $resultado['rcs_rede_social']?></td>
                <td class="editar" >
                    <a href="editarrede.php?codigo=<?php echo $resultado['rcs_codigo'];?>">Editar</a> 
                    <a href="?excluir=<?php echo $resultado['rcs_codigo'];?>">Excluir</a>
                </td>
            </tr>            
        <?php } ?>
         <!--======-- fim ex 1 --=======-->
