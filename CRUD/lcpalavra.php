...
if(isset($_POST['palavra'])){
    extract($_POST);
    if($consulta = $conexao->query("insert into tb_palavras_chaves (pcs_palavra_chave) values ('$palavra');")) {
	} else {
...
if(isset($_GET['excluir'])){
    $codigo = $_GET['excluir'];
    if($consulta2 = $conexao->query("delete from tb_palavras_chaves where pcs_codigo = $codigo")) {
        header("Location: lcpalavra.php");
    } else {
        echo "Erro na exclusão";
        }
    }
$consulta = $conexao->query("select * from tb_palavras_chaves");
?>
    ...
    <div class="container mt-3" id="area-pesquisa"
      ...
      <form class="form" action="?" method="POST" id="form-search">
        <div class="form-group">
          ...
          <!--======----- Área de PESQUISA -----======-->
          <label for="search">Cadastrar nova palavra:</label>
          <input 
          type="text" 
          name="palavra"
          class="form-control" 
    ...
    <div class="container">
      <table class="table table-sm">
        ...
        <tr class="instrucoes">
          ...
          <th class="col-xs-8 col-sm-8 col-md-6" scope="col">Temas</th>

          <th class="col-xs-8 col-sm-8 col-md-6" scope="col">Ações</th>
        </tr><!------------------------------------->
        ...
        <?php while($resultado = $consulta->fetch_assoc()){ ?>
            <tr class="result">
                <td><?php echo $resultado['pcs_codigo']?></td>
                <td><?php echo $resultado['pcs_palavra_chave']?></td>
                <td class="editar" >
                    <a href="editarpalavra.php?codigo=<?php echo $resultado['pcs_codigo'];?>">Editar</a>
                    <a href="?excluir=<?php echo $resultado['pcs_codigo'];?>">Excluir</a>
                </td>
            </tr>            
        <?php } ?>
         <!--======-- fim ex 1 --=======-->
