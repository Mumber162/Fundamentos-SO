...
if(isset($_POST['tema'])){
    extract($_POST);
    if($consulta = $conexao->query("insert into tb_temas (tem_tema) values ('$tema');")) {
	} else {
...
if(isset($_GET['excluir'])){
    $codigo = $_GET['excluir'];
    if($consulta2 = $conexao->query("delete from tb_temas where tem_codigo = $codigo")) {
        header("Location: lctemas.php");
    } else {
    ...
    <div class="container mt-3" id="area-pesquisa"
      ...
      <form class="form" action="?" method="POST" id="form-search">
        <div class="form-group">
          ...
          <!--======----- Área de PESQUISA -----======-->
          <label for="search">Cadastrar novo tema:</label>
          <input 
          type="text" 
          name="tema"
          class="form-control" 
    ...
    <div class="container">
      <table class="table table-sm">
        ...
        <?php while($resultado = $consulta->fetch_assoc()){ ?>
            <tr class="result">
                <td><?php echo $resultado['tem_codigo']?></td>
                <td><?php echo $resultado['tem_tema']?></td>
                <td class="editar" >
                  <a href="editartemas.php?codigo=<?php echo $resultado['tem_codigo'];?>">Editar</a> 
                  <a href="?excluir=<?php echo $resultado['tem_codigo'];?>">Excluir</a>
                </td>
            </tr>            
        <?php } ?>
         <!--======-- fim ex 1 --=======-->
